...
class UniqueSortedStreamTest {
	...
	void testBasicIntegerStream() {
		...
		Stream stream = Stream
				.of(1, 1, 1, 2, 3, 4, 4, 4, 4, 10, 12, 12, 15, 20, 20, 40, 42, 43, 100, 101, 101)
				.map(Integer::toUnsignedLong).map(Row<Object>::new);

		Stream<Row<Object>> uniqSortedStream = UniqueSortedStream.uniqueRowsSortedByPK(stream);
	...
	void testAlreadyUniqIntegerStream() {
		...
		Stream<Row<Object>> stream = Stream.of(1, 2, 3, 4, 10, 12, 15, 20, 40, 42, 43, 100, 101)
				.map(Integer::toUnsignedLong).map(Row<Object>::new);
